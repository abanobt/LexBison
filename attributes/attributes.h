...
const char* node_type_names[] = { 
    "Prog", "DeclBlock", "StmtBlock", "Decl", "Type", "VarList", "VarDecl",
    "Stmt", "PrintStmt", "BlockIfStmt", "SimpleIfStmt", "SimpleStmt", "AssignStmt",
    "ExprList", "RelExpr", "Expr", "MultExpr", "TermExpr", "SFactor", "Factor", 
    "Var", "IDENT", "ICONST", "RCONST", "SCONST", "Len"
};
...
const char* op_names[] = { "plus", "minus", "pow", "mult", "div", "cat", "eq", "lthan", "gthan" };
...
void print_ast_node(ASTNode* node, int indentation) {
    ...
    printf("↳%s ", node_type_names[node->type]); // node_type_names[node->type];
    // Print attribtues
    switch (node->type) {
        case NODE_TYPE:
            printf("(type=%s)", data_type_names[node->actual_type]);
            break;
        case NODE_REL_EXPR:
        case NODE_EXPR:
        case NODE_MULT_EXPR:
...
// Expand the node by adding an additional child node
ASTNode* expand_node(ASTNode* node, ASTNode* add_child) {
    ...
    expanded_children[expanded_count - 1] = add_child;
